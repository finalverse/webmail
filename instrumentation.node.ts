...
console.info(`NuwaMail v${current}`);
...
migrateLegacyAdminLayout()
  ...
  .then(async () => {
    ...
    // admin UI, the NUWAMAIL_TELEMETRY env var, or by clearing the endpoint.
  ...
  .then(async () => {
    // Hourly check against version.telemetry.nuwamail.com. Disable with
    // NUWAMAIL_UPDATE_CHECK=off or override the endpoint with
    // NUWAMAIL_UPDATE_CHECK_URL.
